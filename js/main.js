...
;(function(window) {
	...
	var mainContainer = document.querySelector('.view'),
		...
		loadMoreCtrl2 = mainContainer.querySelector('button.button--view'),
	...
	function loadNextItems() {
		...
		// the timeout serves to simulate the time that we would probably wait for the response
			...
			$.ajax({
				type: "get",
				url:"http://www.3y14.com:8080/image/list",
				async: false,
				data:{
					page:page,
					size:size,
				...
				if (data.code==0){
					...
					if (data.data.length>0){
						for (var i in data.data){
							let a = data.data[i];
							var ele = '<li class="grid__item grid__item--hidden"><a class="grid__link" href="javascript:void(0)"><img class="grid__img" src="'+
								a.url+'" alt="Some image" /><h3 class="grid__item-title">'+a.title+'</h3></a></li>';
							dummyContent=dummyContent+ele;
						}
				...
				error:function () {
					alert("哎呦,服务器出问题了，请联系我吧！")
				}

		})
			var prompt = "<li><h4 class=\"jq22-content\">就这些！！嫌少就点击右侧按钮添加图片</h4></li>"
			if (dummyContent == "" && requestResult){
				dummyContent = prompt;
			}
			...
			if (hasImage) {
				classie.removeClass(loadMoreCtrl, 'button--loading');
				classie.removeClass(loadMoreCtrl, 'button--hidden');
			}
	}
